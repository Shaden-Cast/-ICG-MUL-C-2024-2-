...
class Linea {
    ...
    drawBresenham() {
        let x1 = this.#puntoInicio.x; // Cambiar a let
        let y1 = this.#puntoInicio.y; // Cambiar a let
        const x2 = this.#puntoFin.x;   // Mantener como const
        const y2 = this.#puntoFin.y;   // Mantener como const
    ...
    drawPoint(x, y) {
        ...
        point.setAttribute("r", 2); // Tamaño del punto
...
class Circunferencia {
    constructor(centro, r, color = "red", width = 2) {
...
class Elipse {
    constructor(centro, rx, ry, color = "orange", width = 2) {
        this.#centro = centro; // Instancia de Punto para el centro
...
const circunferencia = new Circunferencia(centroCircunferencia, 50, "red", 2);
circunferencia.draw();

const centroElipse = new Punto(400, 300);
const elipse = new Elipse(centroElipse, 80, 50, "orange", 2);
elipse.draw();
